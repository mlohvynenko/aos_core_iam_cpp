...
class CertHandlerItfMock : public CertHandlerItf {
public:
    MOCK_METHOD(Error, GetCertTypes, (Array<StaticString<cCertTypeLen>>&), (override));
...
namespace aos::cryptoutils {
class CertLoaderItfMock : public CertLoaderItf {
public:
    MOCK_METHOD(Error, Init, (crypto::x509::ProviderItf&, pkcs11::PKCS11Manager&), (override));
    MOCK_METHOD(
...
{
    Config                                    config;
    aos::iam::certhandler::CertHandlerItfMock certHandlerMock;
    aos::cryptoutils::CertLoaderItfMock       certLoaderMock;
    const bool                                provisioningMode {false};

    EXPECT_CALL(certHandlerMock, GetCertificate).WillOnce(Return(aos::ErrorEnum::eFailed));
    EXPECT_CALL(certLoaderMock, LoadCertsChainByURL).Times(0);

    auto err = mRemoteIAMClientMock.Init(config, certHandlerMock, certLoaderMock, provisioningMode);
    ASSERT_TRUE(err.Is(aos::ErrorEnum::eInvalidArgument)) << err.Message();
}

TEST_F(IAMClientTest, InitInProvisioningMode)
{
    Config                                    config;
    aos::iam::certhandler::CertHandlerItfMock certHandlerMock;
    aos::cryptoutils::CertLoaderItfMock       certLoaderMock;
    const bool                                provisioningMode {true};

    EXPECT_CALL(certHandlerMock, GetCertificate).Times(0);
    EXPECT_CALL(certLoaderMock, LoadCertsChainByURL).Times(0);
...
/***********************************************************************************************************************
 * GetRemoteNodes tests
 **********************************************************************************************************************/

TEST_F(IAMClientTest, GetRemoteNodes)
{
    const auto result = mRemoteIAMClientMock.GetRemoteNodes();
