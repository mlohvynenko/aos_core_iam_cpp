...
public:
    ...
    void WaitNodeInfo(const std::chrono::seconds& timeout = std::chrono::seconds(4))
    {
        ...
        mNodeInfoLock.wait_for(guard, timeout);
    }
    ...
    void WaitResponse(const std::chrono::seconds& timeout = std::chrono::seconds(4))
    {
        ...
        mResponseLock.wait_for(guard, timeout);
    }
...
private:
    ...
    std::mutex              mMutex;
    std::condition_variable mNodeInfoLock;
    std::condition_variable mResponseLock;
...
protected:
    ...
    static Config GetConfig()
    {
        ...
        config.mCACert                    = "/etc/ssl/certs/Aos_Root_CA.pem";
        ...
        config.mNodeConnectTimeout    = std::chrono::seconds(1);
...
TEST_F(IAMClientTest, StartProvisioning)
{
    ...
    // StartProvisioning
    EXPECT_CALL(mNodeInfoProvider, GetNodeInfo).WillOnce(DoAll(SetArgReferee<0>(nodeInfo), Return(ErrorEnum::eNone)));
    EXPECT_CALL(mProvisionManager, StartProvisioning(cPassword))
        .WillOnce(DoAll(Invoke(client.get(), &IAMClient::OnEncryptDisk),
            Invoke(client.get(), &IAMClient::OnStartProvisioning), Return(ErrorEnum::eNone)));
...
TEST_F(IAMClientTest, StartProvisioningExecFailed)
{
    ...
    EXPECT_CALL(mProvisionManager, StartProvisioning(cPassword))
        .WillOnce(DoAll(Invoke(client.get(), &IAMClient::OnEncryptDisk),
            Invoke(client.get(), &IAMClient::OnStartProvisioning), Return(ErrorEnum::eFailed)));
...
TEST_F(IAMClientTest, FinishProvisioning)
{
    ...
    EXPECT_CALL(mProvisionManager, FinishProvisioning(cPassword))
        .WillOnce(DoAll(Invoke(client.get(), &IAMClient::OnFinishProvisioning), Return(ErrorEnum::eNone)));
...
TEST_F(IAMClientTest, Deprovision)
{
    ...
    EXPECT_CALL(mProvisionManager, Deprovision(cPassword))
        .WillOnce(DoAll(Invoke(client.get(), &IAMClient::OnDeprovision), Return(ErrorEnum::eNone)));
