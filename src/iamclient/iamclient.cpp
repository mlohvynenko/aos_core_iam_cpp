...
static common::v1::ErrorInfo ConvertAosErrorToProto(aos::Error error)
{
    ...
    if (!error.IsNone()) {
        ...
        if (error.Errno() != 0) {
            message += " [" + std::to_string(error.Errno()) + "]";
        }

        if (error.FileName() != nullptr) {
            message += " (" + std::string(error.FileName()) + ":" + std::to_string(error.LineNumber()) + ")";
        }
    ...
    bool provisioningMode)
{
    ...
    mConnectTimeout            = config.mNodeConnectTimeout;
...
aos::Error IAMClient::OnStartProvisioning(const aos::String& password)
{
    (void)password;

    return ExecCommand("Start provisioning", mStartProvisioningCmdArgs);
}

aos::Error IAMClient::OnFinishProvisioning(const aos::String& password)
{
    (void)password;

    return ExecCommand("Finish provisioning", mFinishProvisioningCmdArgs);
}

aos::Error IAMClient::OnDeprovision(const aos::String& password)
{
    (void)password;

    return ExecCommand("Deprovision", mDeprovisionCmdArgs);
}

aos::Error IAMClient::OnEncryptDisk(const aos::String& password)
{
    (void)password;

    return ExecCommand("Encrypt disk", mDiskEncryptionCmdArgs);
}

IAMClient::~IAMClient()
{
    {
        std::unique_lock guard {mShutdownLock};

        mShutdown = true;
        mShutdownEvent.notify_all();
...
std::unique_ptr<grpc::ClientContext> IAMClient::CreateClientContext()
{
    ...
    ctx->set_deadline(std::chrono::system_clock::now() + mConnectTimeout);
...
bool IAMClient::RegisterNode(const std::string& url, const std::shared_ptr<grpc::ChannelCredentials>& credentials)
{
    std::unique_lock guard {mShutdownLock};
...
void IAMClient::ConnectionLoop() noexcept
{
    ...
    while (!mShutdown) {
        ...
        if (RegisterNode(mServerURL, mCredentials)) {
            if (SendNodeInfo()) {
        ...
        mShutdownEvent.wait_for(guard, mReconnectInterval, [this]() { return mShutdown.load(); });
    }
...
aos::Error IAMClient::ExecCommand(const std::string& cmdName, const std::vector<std::string>& cmdArgs)
{
    if (!cmdArgs.empty()) {
        std::string                    output;
...
aos::Error IAMClient::ExecProcess(const std::string& cmd, const std::vector<std::string>& args, std::string& output)
{
    Poco::Pipe            outPipe;
