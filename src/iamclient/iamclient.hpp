 ...
 * GRPC IAM client.
 */
class IAMClient : public aos::iam::provisionmanager::ProvisionManagerCallback {
public:
    /**
...
private:
    ...
    bool RegisterNode(const std::string& url, const std::shared_ptr<grpc::ChannelCredentials>& credentials);
    ...
    aos::Error ExecCommand(const std::string& cmdName, const std::vector<std::string>& cmdArgs);
    ...
    std::chrono::seconds                      mReconnectInterval;
    ...
    std::condition_variable mShutdownEvent;
