...
aos::RetWithError<Config> ParseConfig(const std::string& filename)
{
    ...
    try {
        ...
        config.mNodeConnectTimeout
            = std::chrono::seconds(object.GetOptionalValue<int>("nodeConnectTimeout").value_or(30));
        config.mNodeReconnectInterval
            = std::chrono::seconds(object.GetOptionalValue<int>("nodeReconnectInterval").value_or(10));
